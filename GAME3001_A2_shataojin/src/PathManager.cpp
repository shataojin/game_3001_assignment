...
void PathManager::GetAllPath(PathNode* start, PathNode* goal) // Updated for Lab 5.
{
	...
	else
	{
		...
		while (currentRecord->m_node != start)
		{
			s_allpath.push_back(currentRecord->m_connection);
			currentRecord = currentRecord->m_fromRecord;
		}
		// Reverse the path.
		std::reverse(s_allpath.begin(), s_allpath.end());
	}
...
void PathManager::DrawPath()
{
	...
	{
		if (s_allpath.empty())return;
		const auto offsetx = glm::vec2(s_allpath[0]->GetFromNode()->GetTile()->GetWidth() / 2,
			s_allpath[0]->GetFromNode()->GetTile()->GetHeight() / 2);
		for (const auto& connection : s_allpath)
		{
			Util::DrawLine(connection->GetFromNode()->GetTile()->GetTransform()->position + offsetx,
				connection->GetToNode()->GetTile()->GetTransform()->position + offsetx,
				glm::vec4(5, 5, 5, 5));
		}
	}
...

	{
		...
		const auto offset = glm::vec2(s_path[0]->GetFromNode()->GetTile()->GetWidth() / 2+2,
			s_path[0]->GetFromNode()->GetTile()->GetHeight() / 2 + 2);
...
std::vector<PathConnection*> PathManager::s_allpath;