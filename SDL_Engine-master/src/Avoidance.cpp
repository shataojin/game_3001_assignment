...
avoidance::avoidance() : m_startPos(glm::vec2(400.0f, 400.0f)),
m_maxSpeed(20.0f), m_turnRate(5.0f), m_accelerationRate(2.0f)
{
	...
	SetType(GameObjectType::AVOIDANCE);
}
...
void avoidance::SetMaxSpeed(float speed)
{
	m_maxSpeed = speed;
}

void avoidance::SetTurnRate(float angle)
{
	m_turnRate = angle;
}

void avoidance::SetDesiredVelocity(glm::vec2 target_position)
{
	m_desiredVelocity = Util::Normalize(target_position - GetTransform()->position) * m_maxSpeed;
}

void avoidance::SetAccelerationRate(float rate)
{
...
void avoidance::LookWhereIAmGoing(glm::vec2 target_direction)
{
	//old version
...
void avoidance::CheckBounds() {}

void avoidance::Reset()
{
	GetTransform()->position = m_startPos;
}
